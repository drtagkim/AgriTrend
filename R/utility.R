...
run_analysis_normal_pack <- function(chr_data,folder_name="") {
  fname_01 <- paste(folder_name,chr_data,"_(100gram).xlsx",sep="")
  fname_02 <- paste(folder_name,chr_data,"_(unit count).xlsx",sep="")
  ddset <- eval(parse(text=chr_data))
  result01 <- pack_month_100g(ddset)
  result02 <- pack_month_c(ddset)
  save.xlsx(fname_01,result01)
  save.xlsx(fname_02,result02)
}
...
run_analysis_subgroup_pack <- function(chr_data,folder_name="") {
  #data
  ddset <- eval(parse(text=chr_data))
  subgroups <- unique(ddset$detail3)
  subgroups[subgroups == "null"] <- NULL #remove null
  rv <- foreach(sg = subgroups) %do% {
    fname_01 <- paste(folder_name,"[",sg,"]",chr_data,"_(100gram).xlsx",sep="")
    fname_02 <- paste(folder_name,"[",sg,"]",chr_data,"_(unit count).xlsx",sep="")
    result01 <- pack_month_100g(ddset)
    result02 <- pack_month_c(ddset)
    cat("Exporting ",sg,"\n")
    cat("=============================\n")
