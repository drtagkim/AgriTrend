...
store_year <- function(dset) {
  ...
  dddset <- dddset0 %>% group_by(year,retail) %>% summarize(amount_mean=sum(f*amount_mean0)/N,price_mean=sum(f*price_mean0)/N,N=n())                                                       
  mirror <- expand.grid(year=2010:2014,retail=c("대형마트", "재래시장", "대형슈퍼마켓","소형슈퍼마켓","직거래"))
  ddddset <- mirror %>% left_join(dddset,by=c("year","retail"))
...
store_year_processed <- function(dset) {
  ...
  dddset() <- ungroup(dddset0)
  dddset <- dddset0 %>% group_by(year,retail,processed) %>% summarize(amount_mean=sum(f*amount_mean0)/N,price_mean=sum(f*price_mean0)/N,N=n())
  mirror <- expand.grid(year=2010:2014,retail=c("대형마트", "재래시장", "대형슈퍼마켓","소형슈퍼마켓","직거래"),
                        processed=c("가공","신선"))
...
income_year <- function(dset) {
  ...
  dddset() <- ungroup(dddset0)
  dddset <- dddset0 %>% group_by(year,income_new) %>% summarize(amount_mean=sum(f*amount_mean0)/N,price_mean=sum(f*price_mean0)/N,N=n())
  mirror <- expand.grid(year=2010:2014,income_new=c("고소득", "중소득", "저소득"))
  ddddset <- mirror %>% left_join(dddset,by=c("year","age_new"))
  ddddset[is.na(ddddset)] <- 0
  ddddset
}
...
income_year_processed <- function(dset) {
  ...
  dddset0 <- ungroup(dddset0)
  dddset <- dddset0 %>% group_by(year,income_new,processed) %>% summarize(amount_mean=sum(f*amount_mean0)/N,price_mean=sum(f*price_mean0)/N,N=n())
  mirror <- expand.grid(year=2010:2014,income_new=c("고소득", "중소득", "저소득"),
                        processed=c("가공","신선"))
  ddddset <- mirror %>% left_join(dddset,by=c("year","age_new","processed"))
  ddddset[is.na(ddddset)] <- 0
  ddddset
}
#' Age Year
age_year <- function(dset) {
  ...
  dddset0 <- ungroup(dddset0)
  dddset <- dddset0 %>% group_by(year,age_new) %>% summarize(amount_mean=sum(f*amount_mean0)/N,price_mean=sum(f*price_mean0)/N,N=n())
...
age_year_processed <- function(dset) {
  ...
  dddset0 <- ungroup(dddset0)
  dddset <- dddset0 %>% group_by(year,age_new,processed) %>% summarize(amount_mean=sum(f*amount_mean0)/N,price_mean=sum(f*price_mean0)/N,N=n())
  mirror <- expand.grid(year=2010:2014,age_new=c("30대이하", "40대연령", "50대연령", "60대이상"),
                        processed=c("가공","신선"))
  ddddset <- mirror %>% left_join(dddset,by=c("year","age_new","processed"))
  ddddset[is.na(ddddset)] <- 0
...
job_year <- function(dset) {
  ...
  dddset0 <- ungroup(dddset0)
  dddset <- dddset0 %>% group_by(year,full_housewife) %>% summarize(amount_mean=sum(f*amount_mean0)/N,price_mean=sum(f*price_mean0)/N,N=n())  
  mirror <- expand.grid(year=2010:2014,full_housewife=c("전업주부","취업주부"))
  ddddset <- mirror %>% left_join(dddset,by=c("year","full_housewife"))
  ddddset[is.na(ddddset)] <- 0
  ddddset
}
#' Job Year Processed
job_year_processed <- function(dset) {
  ...
  dddset0 <- ungroup(dddset0)
  dddset <- dddset0 %>% group_by(year,full_housewife,processed) %>% summarize(amount_mean=sum(f*amount_mean0)/N,price_mean=sum(f*price_mean0)/N,N=n())   
  mirror <- expand.grid(year=2010:2014,full_housewife=c("전업주부","취업주부"),
                        processed = c("가공","신선"))
  ddddset <- mirror %>% left_join(dddset,by=c("year","full_housewife","processed"))
  ddddset[is.na(ddddset)] <- 0
  ddddset
}
#' Brand Year
brand_year <- function(dset) {
  ...
  dddset0 <- ungroup(dddset0)
  dddset <- dddset0 %>% group_by(year,normal_brand) %>% summarize(amount_mean=sum(f*amount_mean0)/N,price_mean=sum(f*price_mean0)/N,N=n())     
  mirror <- expand.grid(year=2010:2014,normal_brand=c("Normal","Brand"))
  ddddset <- mirror %>% left_join(dddset,by=c("year","normal_brand"))
  ddddset[is.na(ddddset)] <- 0
  ddddset
}
#' Brand Year Store
brand_year_store <- function(dset) {
  ...
  dddset0 <- ungroup(dddset0)
  dddset <- dddset0 %>% group_by(year,normal_brand,retail) %>% summarize(amount_mean=sum(f*amount_mean0)/N,price_mean=sum(f*price_mean0)/N,N=n())   
  mirror <- expand.grid(year=2010:2014,normal_brand=c("Normal","Brand"),
                        retail=c("대형마트","재래시장","대형슈퍼마켓","소형슈퍼마켓","직거래"))
  ddddset <- mirror %>% left_join(dddset,by=c("year","normal_brand","retail"))
  ddddset[is.na(ddddset)] <- 0
  ddddset
}
#' Natural Year
natural_year <- function(dset) {
  ...
  dddset0 <- ungroup(dddset0)
  dddset <- dddset0 %>% group_by(year,natural) %>% summarize(amount_mean=sum(f*amount_mean0)/N,price_mean=sum(f*price_mean0)/N,N=n())     
  mirror <- expand.grid(year=2010:2014,natural=c("친환경_유기농","친환경_무농약","친환경_저농약","친환경_일반"))
  ddddset <- mirror %>% left_join(dddset,by=c("year","natural"))
  ddddset[is.na(ddddset)] <- 0
  ddddset
}
#' Pack Month Gram
pack_month_g <- function(dset) {
  ...
  dddset0 <- ungroup(dddset0)
  dddset <- dddset0 %>% group_by(year,month,pack_category) %>% summarize(amount_mean=sum(f*amount_mean0)/N,price_mean=sum(f*price_mean0)/N,N=n()) 
  mirror <- expand.grid(year=2010:2014,month=1:12,pack_category=c("1kg초과","500g이하","500g초과1kg이하"))
  ddddset <- mirror %>% left_join(dddset,by=c("year","month","pack_category"))
  ddddset[is.na(ddddset)] <- 0
  ddddset
}
...
pack_month_g_store <- function(dset) {
  ...
  dddset0 <- ungroup(dddset0)
  dddset <- dddset0 %>% group_by(year,month,pack_category,retail) %>% summarize(amount_mean=sum(f*amount_mean0)/N,price_mean=sum(f*price_mean0)/N,N=n())   
  mirror <- expand.grid(year=2010:2014,month=1:12,pack_category=c("1kg초과","500g이하","500g초과1kg이하"),
                        retail=c("대형마트","재래시장","대형슈퍼마켓","소형슈퍼마켓","직거래"))
  ddddset <- mirror %>% left_join(dddset,by=c("year","month","pack_category","retail"))
  ddddset[is.na(ddddset)] <- 0
  ddddset
}
#' Pack Month Litter
...
fresh_year <- function(dset) {
  ...
  # weighted average
  dddset0 <- ddset %>% group_by(year,panel_c2,fresh) %>% summarize(amount_mean0=mean(purchase,na.rm=T),price_mean0=mean(purchase/quantity,na.rm=T),f=n())
  dddset0 <- ungroup(dddset0)
  dddset <- dddset0 %>% group_by(year,fresh) %>% summarize(amount_mean=sum(f*amount_mean0)/N,price_mean=sum(f*price_mean0)/N,N=n())
  mirror <- expand.grid(year=2010:2014,month=1:12,fresh=c("신선식품","가공식품"))
  ddddset <- mirror %>% left_join(dddset,by=c("year","month","fresh"))
  ddddset[is.na(ddddset)] <- 0
  ddddset
}
#' Fresh Month
fresh_month <- function(dset) {
  ...
  dddset0 <- ungroup(dddset0)
  dddset <- dddset0 %>% group_by(year,month,fresh) %>% summarize(amount_mean=sum(f*amount_mean0)/N,price_mean=sum(f*price_mean0)/N,N=n())  
  mirror <- expand.grid(year=2010:2014,month=1:12,fresh=c("신선식품","가공식품"))
  ddddset <- mirror %>% left_join(dddset,by=c("year","month","fresh"))
  ddddset[is.na(ddddset)] <- 0
  ddddset
